...
const MenuApp: React.FC = () => {
    ...
    return (
        <div className="container">
            ...
            <div className="card">
                <button className="btn" onClick={fetchRandomItem}>
                    랜덤 메뉴 뽑기
                </button>
                {randomItem && (
                    <div>
                        <a
                            href={`https://www.10000recipe.com/recipe/list.html?q=${randomItem}`}
                            target="_blank"
                            className="result green"
                            onClick={() => navigator.clipboard.writeText(randomItem)}
                        >
                            {randomItem}
                        </a>
                    </div>
            ...
            <div className="card">
                <button className="btn" onClick={fetchRandomWeeklyMenu}>
                    랜덤 식단 짜기
                </button>
                <button className="btn" onClick={fetchWeeklyMenu}>
                    급식 식단 짜기
                </button>
                {randomWeeklyMenu && (
                    <div>
                        ...
                        <ul>
                            {randomWeeklyMenu.map((menu, index) => (
                                <li key={index}>
                                    <h3>{days[index]}</h3>
                                    {Array.isArray(menu) ? menu.map((item, i) => <div key={i}>{item}</div>) : menu}
                                </li>
                            ))}
                        </ul>
                    </div>
            ...
            <div className="card">
                ...
                <div className="category-description">
                    <p>
                        {' '}
                        키워드 버튼을 누를 때마다 키워드별 메뉴가 다섯개씩 랜덤으로 추천됩니다. <br /> 추천된 메뉴를
                        눌러 메뉴를 클립보드에 복사하고 만개의 레시피 사이트에서 검색할 수 있습니다.
                    </p>
                </div>
                <div>
                    <input
                    ...
                    />
                    <button className="btn" onClick={() => updateCategories()}>
                        +
                    </button>
                ...
                <div className="keywords-grid">
                    {categories.map((cat, index) => (
                        <button
                            className={`btn category ${editMode ? 'edit-mode' : ''}`}
                            onClick={() => {
                                if (editMode) {
                                    removeCategory(cat);
                                } else {
                ...
                <div className="recommendation-grid">
                    {categoryItems.map((item, index) => (
                        <a
                            href={`https://www.10000recipe.com/recipe/list.html?q=${item}`}
                            target="_blank"
                            key={index}
            ...
            <div className="card">
                ...
                />
                <button className="btn red" onClick={addKeyword}>
                    키워드 빼기
                </button>
                        ...
                        <div className="keywords-grid">
                            {ignoredKeywords.map((kw, index) => (
                                <div key={index} className="keyword-item">
                                    <button className="btn red" onClick={() => removeKeyword(kw)}>
                                        {kw} x
                                    </button>
                                </div>
                            ))}
